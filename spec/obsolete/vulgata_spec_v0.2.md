# Vulgata Design Specification version 0.2
...
For v0.2, all top-level declarations are module-public by default. A later version may add `private`.
...
Block comments are omitted in v0.2 to keep lexing simple.
...
Actions are not equatable in v0.2.
...
Outside tests, `expect` is legal only if the execution environment permits assertions in production mode. For v0.2, it is best limited to tests.
...
A design decision is needed on mutable versus persistent collection operations. For the first implementation, explicit mutation through `set` on indexed or variable targets is sufficient. Standard library collection helpers may return new values rather than mutating in place.
...
Do not introduce exceptions in v0.2.
...
Variables declared with `let` are mutable bindings only if later `set` assigns to them. There is no separate `var`.

## 14.2 Records and collections

For v0.2, records and collections are mutable at the language level through explicit `set`.
...
In compiler mode, the generated Rust code may use mutable structs and collections.
In interpreter mode, runtime values may be mutable heap values.

## 14.3 Aliasing

Aliasing semantics must be defined conservatively:

* assigning a record or collection value binds a reference-like managed value in interpreter mode
* compiler mode should preserve equivalent observable semantics

This means the language behaves as a managed high-level language, not by-value copying everywhere.

---

## 15. Full grammar proposal

Below is the expanded grammar for v0.2.
...
set_stmt        = "set" target "=" expr NEWLINE ;
...
No function overloading in v0.2.
...
1. Whether generics are fully implemented in v0.2 or partially reserved.
...
* record field
* list element
* map entry if mutable indexing is supported in the first runtime
...
* assignment target must be writable
...
Interpreter semantics should treat records, lists, and maps as managed mutable values. Assignment and updates must have observable aliasing behavior consistent with compiled execution.
